...
**Strictly rate limited** (240/min per IP, 120/min per session — DM-042),
including when the caller authenticates with an ApiKey. This endpoint becomes
a DB write per call, so a client that reports one event per failure inside a
retry loop can turn its own error handling into an amplifier. Sample and
dedupe repeated errors, and buffer/batch where possible — see
