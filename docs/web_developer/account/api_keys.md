...
**Deactivating the key's group instantly suspends the key** — every group-scoped request (list, detail, save, delete, with or without a `group=` param) is denied while the group is inactive. The key is never modified, so reactivating the group restores it immediately; you do not need to (and should not have to) deactivate the key itself. The key still *authenticates* (so the group-independent federation-sync path keeps working) — it simply has no group context. An active child group under an inactive parent remains reachable via an explicit `group=<child id>`.
