...
**`request.user` is never `None`.** When no bearer token resolves, `request.user`
is the `ANONYMOUS_USER` sentinel (`is_authenticated=False`), not `None` — so
`request.user.is_authenticated` and `request.user.has_permission(...)` are
