...
AWS-supplied temporary addresses.

**This package has no cutover command.** It never transfers, releases or
reattaches an elastic IP, and it never remaps a live balancer's subnets. Every
client it builds refuses `DisassociateAddress`, `ReleaseAddress` and
`SetSubnets` by name, on top of the ordinary destructive-prefix guard;
brownfield's positive allowlist also refuses `AssociateAddress`. (Managed
greenfield stable-node EIPs keep their existing association behavior — that is
a different, non-live path.) Moving a public address onto the prepared fleet is
external operator work, performed deliberately elsewhere, by a human.

`manage_dns` is required in every brownfield manifest and must be the literal
`false`. Missing is not treated as false.

#### Declaring addresses that are held elsewhere

One optional manifest field exists, and it changes what preparation *creates*,
never what it *moves*:
...
Declaring it says: *these elastic IPs are already attached to something outside
this fleet.* Preparation responds by building the shadow NLB on AWS temporary
addresses in the normal two subnets, instead of allocating replacement EIPs
that would sit unused beside addresses production already owns. It does not
allocate, tag, attach or adopt the declared allocations, and no AWS call in
this package reads the mapping — only its presence. The exact map is validated
operator evidence of which addresses are spoken for.

Omitting the field is valid and means "nothing is spoken for": preparation
follows the ordinary fixed-address path. A present value is validated exactly,
because a manifest naming the wrong AZ or a typo'd allocation is evidence of
nothing. It must be a non-empty object mapping one or both selected NLB
subnets' availability zones to unique `eipalloc-*` ids, at most one per AZ. An
empty object, a duplicate allocation, an id that is not an `eipalloc-*` string,
a JSON list or object where an id belongs, or an AZ outside the selected NLB
subnets is refused by field name before any AWS access.

One preserved allocation can maintain one public-IP continuity path; it does
**not** create two customer-known ingress addresses. Fixed-IP clients remain
effectively single-ingress/single-AZ until a later edge expansion. Do not
represent one preserved address as a redundant edge. DNS providers that can
point at the NLB name may move independently; providers pinned to an IP need
the external cutover below.

#### Cutover and its reverse are external, and their evidence is yours

Because django-mojo performs no cutover, it also records none. Whoever moves
the address or the DNS record owns the whole procedure: the rehearsal, the
change window, the canaries, the recorded before-and-after state, and a tested
reverse procedure that can put the address or the record back. None of that
evidence is produced, stored or verified by this package, so it must be
captured independently — a `fleet-status` run proves the fleet is ready, and
proves nothing about a cutover.

Note that removing an NLB subnet terminates that AZ's active connections, and
AWS says it can take up to three minutes; clients using a transferred IP can
fail briefly between disassociation and reassociation. Plan the window
accordingly.
...
- There is no rollback-by-delete. Before public cutover, recovery is simply to
  leave production DNS/EIPs on the existing servers — preparation touched
  neither. After an external cutover, recovery is that procedure's own reverse
  step, which is the operator's to write and to have tested.
