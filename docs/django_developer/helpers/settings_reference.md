...
- `DATABASE_POOL_LAB_TRACE_LEASES` — **file-only**, default false/absent.
  Enables MojoLand's per-worker acquire/return correlation only when its value
  is exactly `True`, the local probe is exactly enabled, and the active pool
  plan is valid `api` / `asgi` with `DATABASE_POOL_IDENTITY.project` equal to
  `mojoland`. Every mismatch disables tracing. Evidence uses only the
  server-owned route labels `group-detail` and `group-apikey-me`, categorized
  thread names, bounded Python stack locations, and return-error class names;
  do not enable it as ordinary production telemetry.
