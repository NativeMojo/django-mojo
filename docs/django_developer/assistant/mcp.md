...
* **`HIDDEN_TOOLS`**, two named sets in `mcp/server.py`, each with one reason.
  ...
  | `_LLM_SPENDING_READS` | `analyze_image` | **A read tool that spends an installation-owned LLM credential is not exposed over MCP.** `analyze_image` sends an image plus a client-chosen free-text prompt through the exact `file_analysis` safety-policy route, whose route selects the credential and model; read-only tools carry no approval gate, so over MCP this would be remotely driven installation spend. Re-run `rg -n "helpers\.llm\.(call|ask)|feature=.*file_analysis" mojo/apps/assistant/services/tools/` when adding tools and add every spending read it finds to this set. |
