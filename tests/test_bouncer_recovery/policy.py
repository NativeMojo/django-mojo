...
def test_hosted_routing_matrix(opts):
    ...
    for page in ('login', 'registration', 'public_message'):
        for raw in (0, 39, 40, 59, 60, 180):
            ...
            expected = 'recovery' if value.decision == 'block' else 'check'
        ...
        assert route(result(180, 80, page=page), page) == 'recovery', 'uncapped retained block cannot be erased by recovery credit'
        ...
        assert route(result(0, page=page), page, matched=True) == 'recovery', 'active signature wins over an otherwise clean score'
