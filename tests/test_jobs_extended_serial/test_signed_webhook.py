"""Signed-webhook handler tests moved out of tests/test_jobs/test_signed_webhook.py.

These call post_webhook directly in this process with the shared
mojo.apps.jobs.handlers.webhook module's `requests` attribute patched, and/or
mutate django.conf.settings via `_override_setting` — both visible to every
parallel test thread — so they run only in the opt-in serial tier (maestro
item #1839). post_webhook takes no injectable transport, so there is no seam
to convert through; the patches wrap direct in-process calls (not opts.client
traffic), so none were deletable no-ops.
"""
