"""Stable outbound IPs: the apply gate, the two runners, and the REST plane.

These tests moved from ``tests/test_aws/capacity_addresses.py`` because they
patch shared module attributes — ``capacity._write_policy``,
``_egress_enabled``, ``_dispatch``, ``_serving``, ``_write_operation``,
``invalidate``, ``_release``, ``_attach_stable_ip``, ``capacity.apply``, the
``mojo.helpers.aws.ec2`` helper module, ``platform_deploy``,
``system_settings``, ``mojo.apps.edge.asyncjobs._publish_deploy_node`` and
...
request-shape (Stubber), report, and validator tests stayed behind in the
default-tier module.
"""
