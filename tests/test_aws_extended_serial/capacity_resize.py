"""Capacity resize guards, runners, and in-process REST wiring.

Moved from tests/test_aws/capacity_resize.py because they patch shared module
attributes (capacity._dispatch/_sleep/_write_operation/invalidate/_release/
time/apply, the mojo.helpers.aws.rds and elasticache helper modules, and
...
two modules never touch each other's shared-cache claim keys.
"""
...
CLUSTER = "mojo-resizex-aurora"
WRITER = f"{CLUSTER}-1"
READER_A = f"{CLUSTER}-2"
READER_B = f"{CLUSTER}-3"
STANDALONE = "mojo-resizex-postgres"
STANDALONE_REPLICA = f"{STANDALONE}-replica"
CACHE_GROUP = "mojo-resizex-redis"
...
def setup_capacity_resize_extended(opts):
