...
def test_failed_unstarted_idempotent_publish_is_resumed(opts):
    ...
    assert resumed_id == first_id, "An unstarted failed publish must resume the durable Job"
...
# test_publish_bookkeeping_failure_does_not_fail_queued_job and
# test_duplicate_redis_delivery_claims_job_once moved to
# tests/test_job_engine_extended_serial/test_publish_execute.py: they patch
# shared mojo.apps.jobs internals (jobs.metrics.record,
# job_engine.load_job_function) in-process, which is unsafe under the parallel
