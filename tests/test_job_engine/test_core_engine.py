...
def test_engine_initialization(opts):
    ...
    # The requested channels are consumed; the engine also adds its own
    # box-direct channel — named after its runner id — so a publisher can
    # address this engine directly (JOBS_HOSTNAME_CHANNEL).
    assert opts.test_channel in engine.channels, \
        f"Expected {opts.test_channel} to be consumed, got {engine.channels}"
    assert set(engine.channels) <= {opts.test_channel, engine.runner_id}, \
        f"Engine should consume only what was asked for plus its runner-id channel, got {engine.channels}"
