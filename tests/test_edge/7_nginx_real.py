...
**When the tier IS selected and nginx is missing, this FAILS — it does not
skip.** That is deliberate and it is the whole reason the file exists: a
renderer whose output nobody has ever handed to nginx is a renderer that will
eventually emit invalid configuration, and a silent skip is exactly how that
ships. Every other test in this module asserts on strings we generated and then
read back, which cannot catch a directive that is well-formed to us and
meaningless to nginx.
...
def test_real_nginx_accepts_every_kind(opts):
    ...
    assert binary, (
        "nginx is not installed on this host, so the generated configuration "
        "has never been parsed by nginx. This FAILS rather than skips on "
        "purpose — a renderer nobody validated is one that will eventually "
        "emit invalid config. Install it (`brew install nginx` / "
        "`apt-get install nginx`) or drop the `extended` tier from this run.")
