"""Moved from tests/test_deploy/nginx_runtime.py (maestro #2558).

Converge/SELinux/reconcile orchestration coverage: these tests mock
`mojo.deploy.nginx_runtime` internals (`nginx_dump`, `_mkdir_exact`,
`_install_fragment`, `runtime.os`, `runtime.shutil`, ...) — process-global
patches of production module attributes, unsafe under the parallel default
tier. The pure fragment/parser/decision contracts stay in the default-tier
tests/test_deploy/nginx_runtime.py.
"""
