"""Config-plane tests moved from tests/test_geofence/config_plane.py —
the generic /api/settings write path posts REAL protected GEOFENCE_* Setting
rows through the live server, which is unsafe under the parallel default tier
(maestro item #1839). Runs opt-in (`extended`) and serial.

These tests write REAL global Setting rows (GEOFENCE_SYSTEM_RULES /
GEOFENCE_ALLOWLIST); every mutating test restores state in `finally`.
"""
...
def setup_config_plane_serial(opts):
    ...
    opts.admin_email = f"geofence_seradmin_{suffix}@geofence.test"
