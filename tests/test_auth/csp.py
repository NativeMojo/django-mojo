...
The settings-behavior tests (direct `build_policy()`/`apply()` calls that
override django.conf.settings in-process) live in
tests/test_auth_extended_serial/csp.py (maestro item #1839) — process-global
settings mutation is unsafe under the parallel default tier.
"""
import re
