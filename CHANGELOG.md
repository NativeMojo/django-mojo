...
- **`Group.member_count`** — new property + `extra` on the `basic` and `default` graphs. Returns the count of active direct members (excludes inactive memberships and descendant groups). Lets list endpoints render member counts without fetching the membership collection.
