...
convention, and it matches the post-ITEM-025 framework norm (deny by default, one
...
- `Group.get_active(pk)` — `mojo/apps/account/models/group.py:223-232` (added by
  ITEM-025): `cls.objects.filter(pk=pk, is_active=True).first()`. Inactive resolves
...
1. **`mojo/apps/account/rest/group.py`** — rewrite `on_group_me_member` (102-118):
   ...
   @md.requires_auth()
   def on_group_me_member(request, pk=None):
       # SECURITY: active-only resolution (inactive == nonexistent, ITEM-025 contract)
...
- **Member of an INACTIVE group** now gets 403 (previously 200 with their record).
  ...
  dispatcher contract from ITEM-025.
