...
hardened by ITEM-018). The create/edit endpoint is
...
ITEM-018 module/style; see Tests).
...
- **memory crash**: a key reaching `_can_read_tier`/`_can_write_tier` returns a
  ...
  apikey identity).
- Keep the ITEM-018 `tests/test_global_perms/apikey_gate.py` green (it already
...
- **Post-review completion (security review, 2026-07-08) — two override
  ...
  2. **WARNING — `Group.check_view_permission`** (my own ITEM-019 edit) gated
