...
- `docs/django_developer/assistant/README.md` — settings table row + new "Prompt Caching" subsection.
- `docs/web_developer/assistant/README.md` — `usage` row in Message fields table.
- `CHANGELOG.md` — v1.2.29 entry (bumped from v1.2.25 by docs-updater agent to avoid version collision).
...
- Full suite: `bin/run_tests --agent` — 2167 passed / 0 failed / 56 skipped (opt-in), 64.8s. No regressions.

### Security Review
No concerns. Reviewed `cache_control` injection (static literal, no user input), `Message.usage` REST graph exposure (gated by `view_admin`, integer counts only), per-turn log content (only `conv=pk` + counters, no PII), `_zero_cache_warned` thread safety (TOCTOU race only causes 2-3x warning fire, not a security issue), and `LLM_ADMIN_PROMPT_CACHE_ENABLED` fail-open default (appropriate for a cost/performance feature, not a security gate).
