...
- **`--full` (opt-in `test_security`) is RED at HEAD, pre-existing, NOT from ITEM-002:**
  - `public_endpoints_security`, `route_security_comprehensive`,
    ...
    entries exist (this decorator is always inner to `requires_auth`), so ITEM-002
  ...
  - `auth/exchange rate-limit` — fails only in the parallel `--full` run (shared
    rate-limit counters); passes default/isolated. Flake, not ITEM-002.
- ITEM-002's own tests all pass: `test_auth.fresh_auth`, `test_user_mgmt.username_change`,
