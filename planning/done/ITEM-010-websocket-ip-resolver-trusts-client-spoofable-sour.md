...
the HTTP `get_remote_ip()` hardened in ITEM-009 — and it prefers client-spoofable sources over
...
set `X-Forwarded-For`/`Forwarded` and control the IP the server records — the same bug ITEM-009
...
runs at connect) — so this is lower-severity than ITEM-009. But it IS a security-forensics
...
inconsistent across transports right after ITEM-009 hardened the HTTP side.

Why now: same-class sibling of the just-closed ITEM-009; closing the WS half makes the
...
**Fix shape (mirrors ITEM-009):** invert priority to prefer `X-Real-IP` (normalized) in both
...
one normalizer with the HTTP path (ITEM-009).
...
- `mojo/helpers/request.py` `_normalize_ip` (added in ITEM-009, ~lines 42-60) — the normalizer;
  its ONLY caller is `get_remote_ip` (HTTP) in the same file (grep `_normalize_ip` to confirm).
  ITEM-009's test (`tests/test_helpers/test_get_remote_ip.py`) exercises the PUBLIC `get_remote_ip`,
...
4. `CHANGELOG.md` — security entry: WS client IP now from `X-Real-IP`; XFF/`Forwarded` no longer
   trusted on the realtime path; consistent with ITEM-009.
...
- **Transport peer as last resort** — the WS analog of ITEM-009's `REMOTE_ADDR` fallback; over the
...
- `self.remote_ip` can be `None` (already possible pre-fix). Consumers tolerate it (logging prints
  it; Redis stores it; `Event.source_ip` pre-existing behavior). Same `None` posture as ITEM-009's
...
- Behavior change: WS-recorded IP now matches HTTP (real client, not the spoof). A deployment whose
  ...
  include sets it.
- Out of scope (separate items): geofence fail-open; nginx/uvicorn DiD; the ITEM-009 `UserLoginEvent`
  None-IP watch item.
...
## Notes
- Build baseline (2026-06-30, `bin/run_tests --agent`, default suite, HEAD includes ITEM-009):
  **GREEN** — 2266 total, 2210 passed, 0 failed, 56 skipped (`testproject/var/test_failures.json`,
  `"failures": []`). `test_realtime` 13/13. Any failure after this change is attributable to ITEM-010.
- Post-build agents (2026-06-30): full default suite green (2216 passed, 0 failed); docs updated in
  ...
  normalizes/stores IPv6, making it more likely to surface (also flagged in the ITEM-009 audit).
  Fix needs a model migration → separate item.

## Resolution
- closed: 2026-06-30
- branch: main
- files changed: CHANGELOG.md,docs/django_developer/core/middleware.md,docs/django_developer/helpers/request.md,docs/django_developer/realtime/architecture.md,docs/web_developer/core/request_response.md,docs/web_developer/realtime/websocket.md,memory.md,mojo/apps/realtime/handler.py,mojo/helpers/request.py,planning/.next_id,planning/done/ITEM-009-get-remote-ip-trusts-client-supplied-x-forwarded-f.md,tests/test_helpers/test_get_remote_ip.py,tests/test_realtime/ip_resolution.py,uv.lock
