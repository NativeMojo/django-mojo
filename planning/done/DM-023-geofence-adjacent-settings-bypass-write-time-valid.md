...
(2026-07-08). DM-017 added write-time validation for DB-backed geofence
...
ALREADY validated — DM-021 made `GEOFENCE_KEYS` a 3-tuple. The genuinely
...
- `tests/test_geofence/config_plane.py:170-204`
  ...
  scoped row → 400 body contains "global-only". DM-021 added the same for
...
1. **`mojo/apps/account/models/setting.py`** — validator registry + save-time
   ...
   - Module-bottom validators + registrations (lazy imports inside the two
     ...
     (DM-021's `strict_posture.py` backdoor test keeps passing). Update the
...
3. **One 8-key family tuple keeps driving cache invalidation.**
   `invalidate_all()` on a rare admin write is cheap; DM-021's resolution
