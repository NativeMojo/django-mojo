...
`_requires_global_perms` (ITEM-017): global `User.permissions`/superuser
...
(ITEM-016; `api_key.py:236-237` binds `request.group`/`request.api_key`).
...
(promote ITEM-017's `_requires_global_perms` verbatim, PLUS the ApiKey
guard):
- after the `is_authenticated` check, reject non-User identities:
  ...
  (fail-closed; canonical idiom per ITEM-016).
...
- `escalation.py` — the core regression, table-driven. Setup: one
  member-granted user (per the ITEM-017 template: real Group + GroupMember +
