...
- Sibling of DM-023 (`GEOFENCE_FAIL_CLOSED_SCOPES` / `ALLOW_PRIVATE_IPS`
...
in `GEOFENCE_KEYS` with validators + cache invalidation (DM-023). Only the
...
- **`report_block` "fail-open allowed" verb sub-finding → separate item, not
  ...
  a blocked-copy workaround) that deserves its own scoping. Keeping DM-031
...
- Post-build agents: test-runner green (2414/2358/0, test_geofence 106/106);
  ...
  **CRITICAL pre-existing sibling** outside this diff: `phone_register.py:102`
  ...
  remotely-armable phone-verification bypass (same vector as DM-031, worse
