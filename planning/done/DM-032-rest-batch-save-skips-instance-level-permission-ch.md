...
shipped model — in particular `Group` (no `CAN_BATCH`), so DM-027's
...
## Investigation
- Surfaced by the DM-027 security review (commit 4137760) while tracing every
...
- **`_evaluate_permission` (rest.py:201-352)** with an instance runs: write
  classification (`SAVE_PERMS` in keys → write, per DM-027, rest.py:254-258),
...
- **DM-027 consistency** (planning/done/, commit 4137760): passing
...
- **Test constraints**: RestMeta mutation does NOT cross into the testit server
  ...
  (DM-027's regression — `Group.objects.create`, `grp.add_member(user)`,
...
- **Exact permission keys copied from the single paths** (`["SAVE_PERMS",
  ...
  "VIEW_PERMS"]` for creates) so DM-027's write classification and hook
