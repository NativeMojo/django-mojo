...
opened: 2026-07-10
depends_on: []
related: []            # ITEM-028 (done, the anon-500 fix that surfaced these), ITEM-016 (choke-point guard)
links: []
---
...
## What & Why
ITEM-028 hardened `ANONYMOUS_USER.has_permission` to `lambda *a, **kw: False`
...
defense-in-depth / hygiene, not a known live vulnerability (post-ITEM-028 these
sites already fail closed with a clean 403).

Sites identified during ITEM-028 (from the security-review and Explore passes):
- `mojo/apps/chat/rest/rooms.py:300` (`_check_room_admin`) and `:312`
  ...
  (the ITEM-028 docs-updater counted ~5 `_check_room_admin`/`_check_room_moderator`
...
- [ ] No behavior change for authenticated callers; anonymous callers continue to
      get a clean 403 (already true post-ITEM-028).
- [ ] If `/scope` finds any site that is anonymously **reachable** AND was still
      returning a raw 500 that ITEM-028's lambda fix did NOT already resolve →
      reclassify this item as a **bug** and add a regression test for that site.

## Investigation — open questions for /scope
- **Reconcile the conflicting ITEM-028 findings (the crux):** the security-review
...
- For `member.can_change_permission`: identify which REST endpoints reach
  ...
  (e.g. `group/member/invite` is now gated by ITEM-028; check the member-update
  and any other paths).
- Confidence: the call sites exist (confirmed in ITEM-028); their reachability /
...
The ITEM-028 concern was a **false alarm**. The ITEM-028 security-review flagged `chat/rest/rooms.py:300`/`:312` and `member.py:87` as "no preceding auth gate," but it read the *helper functions* in isolation. A full per-endpoint recon shows every route that reaches one of these permission checks already carries `@md.requires_auth()` (or a stronger `requires_perms`/`requires_global_perms`), or is behind the model-security 401 short-circuit. ITEM-028's `ANONYMOUS_USER.has_permission = lambda *a, **kw: False` is a redundant fail-closed backstop, not the sole protection. There is nothing to gate.
...
- **`member.can_change_permission`** (`member.py:86-95`) — reached only via `on_group_invite_member` (`group.py:37` `requires_auth`, from ITEM-028) and `on_group_member` (`uses_model_security(GroupMember)`; `SAVE_PERMS` has no `"all"` → `rest.py:234-240` 401-blocks anon before the setter dispatch runs).
...
- Do **not** add redundant `if not request.user.is_authenticated: raise` to the four helpers — pure noise given the callers' gates plus ITEM-028's fail-closed sentinel.

Adjacent, out of scope (noted, not filed): the metrics endpoints in `metrics/rest/{values,categories,base}.py` use `@md.custom_security(...)` + the self-gating `check_view/write_permissions` helpers (`metrics/rest/helpers.py`) rather than a `requires_*` decorator — safe today (helpers check `is_authenticated` before `has_permission`), same "annotation is not a gate" pattern ITEM-028 flagged. File separately only if a uniform decorator posture is later wanted.

## Notes
- Came out of ITEM-028's "Open questions" / post-build agent notes (2026-07-10).
...
- outcome: NO ACTION NEEDED. The ITEM-028 security-review flag was a misread of the helpers in isolation; every reaching endpoint is already authentication-gated (full per-site recon in `## Plan`).
