...
tenant's data. DM-025's active-only resolution only bites when a request
...
the DM-019 groupless-ApiKey branch (`mojo/models/rest.py:288-311`) denies by
...
## Investigation
Traced by the DM-025 post-build security review (2026-07-10) — confidence:
**high** (code-path reading of api_key.py:272-303 + rest.py:288-311; re-verify
during /scope). Pre-existing; DM-025 documented it rather than fixing it
...
- **Strip group context (403 at model security), not a hard 401** — the geoip
  ...
  strip-context does not. Matches DM-025's "inactive == no group context".
...
```python
        if request.group and is_group_scoped:
            ...
            # groupless / null-group: DM-019 deny-by-default
...
**DM-025 contract — `Group.get_active` (`group.py:223-232`)** filters
...
1. **`mojo/apps/account/models/api_key.py`** — `validate_token`, replace line 303:
   ...
   # DM-025's active-only contract. NOT a hard reject: the federation path
...
- **Gate the resolved group, per request** (not cascade-deactivate children):
  minimal, consistent with DM-025, and leaves the documented active-child
  boundary intact.
...
- **`docs/django_developer/`** — the API-keys / permissions page(s) (grep
  ...
  fail-closed + logged).
- **`CHANGELOG.md`** — bug entry under the rolling block (DM-037).
...
- **Baseline (2026-07-12, before any edit):** `bin/run_tests --agent` →
  ...
  touch DM-037's code (`api_key.py`, `rest.py`). Accepted pre-existing set —
