...
opened: 2026-07-08
depends_on: []
related: [DM-017, DM-020, mverify_portal#ITEM-014]
links: []
---
...
DM-017's REST contract (`GET /api/geo/rules`, incident Events) and the
consuming portal page (mverify_portal DM-014) are both, by design,
...
Raised during mverify_portal DM-014's design review (2026-07-08: "is this
...
### Context — what exists (verified 2026-07-08/09; DM-021 has landed — geofence.py/group.py refs match current HEAD; engine.py/evidence.py refs may sit ± a few lines)
...
- `Group.user_has_permission(user, perms, check_user=True)`
  ...
  guard (`hasattr(user, "is_request_user")` — DM-016 choke point), then
...
- **Attribution caveat (DM-020, deliberate):** the geofence evidence calls
  ...
  (deriving group from `user.org` was explicitly rejected in DM-020).
...
2. **Keys = `view_security`/`security` (global or member) via `@md.requires_perms`.**
   ...
   framework; the portal maps its roles to member grants). DM-018's
...
4. **Read persisted config directly (settings + `group.metadata`), mirroring
   ...
   DM-021's in-flight engine work.
5. **Event side: no emission changes.** Member visibility of group-attributed events
   ...
   (DM-020 decision). `geofence_config` stays effectively platform-only (groupless) —
...
- **DM-021 landed (closed 2026-07-09)** — the strict-posture fields cited above are
...
strict=true `Setting` row (DM-021 hygiene — parallel unheadered requests would all
...
- **Not urgent / not blocking**: the platform-admin surface (DM-017 +
  mverify_portal DM-014) already fully serves the compliance-evidence use
