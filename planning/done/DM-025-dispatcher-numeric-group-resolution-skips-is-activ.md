...
DM-020) evidence metrics are attributed to its `group-<id>` account.

Surfaced by the DM-020 post-build security review. Same review's product
...
can be *attributed* activity by an anonymous blocked caller. DM-020
...
### Context — what exists
All line refs current post-DM-024 (which added the `(TypeError, ValueError)` guards in these exact regions — do not disturb that behavior).
...
- **Test fixtures to reuse**:
  ...
  - DM-024's `tests/test_middleware/request_data_merge.py` covers the coercion guards ('Invalid group ID' 400, fail-closed on garbage) — extend, don't duplicate.
...
### Design decisions
- **`Group.get_active(pk)` classmethod, not inline filters at 3 sites** — one owner for a security invariant beats three copies (same "guard the choke point" reasoning as DM-016); `get_model_instance` stays generic because it serves many models.
...
- **Out of scope — anonymous attribution integrity** (the DM-020 product note): callers choosing any *active* group's context on public surfaces is the white-label design; DM-020 already documents per-group counters as "reported activity, not verified counts." This item only closes the inactive slice. Recorded as the deliberate decision the acceptance criteria asked for.
...
- **`group=0` / negative / garbage**: `0` is falsy → branch skipped (unchanged); negative → no match → None (unchanged); non-numeric → DM-024's 400 (unchanged, already tested).
- **ApiKey with its own group deactivated**: numeric `group=` now clobbers to None instead of resolving inactive → groupless fail-closed model security (DM-019). Deny-consistent; no legitimate active-key flow changes.
...
5. **`requires_group_perms` in-process** (no HTTP callers exist): real user with a real GroupMember perm grant in an inactive group; decorate a dummy view, call the wrapper with `objict(user=<real User>, DATA=objict(group=str(inactive_pk)), group=None)` → expect `PermissionDeniedException` (pre-fix: the inactive group resolves, the member grant passes, the dummy executes — test fails). Mirrors DM-024's in-process wrapper test at `tests/test_middleware/request_data_merge.py:178-198`.
...
- `CHANGELOG.md` (Unreleased) — **security** entry: numeric `group=` now matches `group_uuid` (active-only, no touch/oracle on inactive), including the `requires_perms`/`requires_group_perms` fallback. (DM-025)
