...
DM-017's evidence plane records geofence metrics (`geofence:blocks`,
...
`/api/geo/check` and simulate emit nothing (by design, DM-017).
...
- **Cache-hit decisions** still emit evidence (decorator-level, DM-017
...
- **Test hygiene** (memory, DM-017): never prime cached denies under the shared
...
- **Baseline (2026-07-08, pre-edit)**: `bin/run_tests --agent` GREEN —
  ...
  attributable to it.
- Consumer rider: mverify_portal DM-014 (scoped) ships its charts with
