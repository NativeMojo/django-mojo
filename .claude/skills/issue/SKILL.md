...
$ARGUMENTS — The user's issue description. If empty, ask the user to describe the problem.
...
- Trace the code path described in the issue report
- Identify the likely root cause or narrow it to 2-3 candidates

### 3. Best-Effort Confirmation
- If you can confirm the issue through code analysis alone (logic error, missing guard, wrong field name, race condition), state your confidence level
...
**Code path**: <file:line references tracing the issue>
...
- If the issue cannot be confirmed, say so honestly and set confidence to "speculative".
