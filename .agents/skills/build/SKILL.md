...
name: build
description: >-
  Implement a scoped work item from planning/confirmed/ one task at a time,
  with tests. For bugs (type: bug), write a failing regression test before the
  fix. After committing, spawn the post-build agents (test-runner,
  docs-updater, security-review). Use when executing an item that has already
  been scoped.
---

<!-- Generated from .claude/skills/build/SKILL.md. Do not edit directly. -->
...
Read `AGENTS.md` for conventions. Read the item file in `planning/confirmed/`.
...
- **delegate** — spawn ONE builder sub-agent (model = `build_model`) that executes
  ...
  `AGENTS.md`, `.claude/rules/`, and the item file, and state explicitly: the
