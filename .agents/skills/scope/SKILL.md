...
<!-- Generated from .claude/skills/scope/SKILL.md. Do not edit directly. -->
...
Read `AGENTS.md` for project conventions first.

Named `$scope`, **not** `/plan`, to avoid colliding with Codex's built-in
plan mode.
...
1. **Spawn the drafter** — ONE read-only sub-agent, `model: sonnet`, working from
   the confirmed item path. Its prompt must tell it to: read `AGENTS.md`, this
