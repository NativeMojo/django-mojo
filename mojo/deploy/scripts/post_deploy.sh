...
# certificate path values. Preservation needs one TLS server on both sides, the
# same normalized server_name set, one canonical same-lineage Certbot pair, and
# root-safe installed material. The child owns validation, the final snapshot
# check, and os.replace(), keeping the no-follow decision and mutation inside
# one trusted-change operation.
#
# The child's logic lives in mojo/deploy/vhost_install.py — a real module with
# real unit tests, not a heredoc. VHOST_PY is resolved once before the vhost
# loop; an empty VHOST_PY means the installed framework predates the module
# (this file's inode outlives the pip install, see the header) and we fall back
# to install_file semantics.
install_vhost() {
    local src="$1" dest="$2" name
    name="$(basename "$dest")"
    [[ -f "$src" ]] || die "expected vhost source is missing"
    if [ -z "$VHOST_PY" ]; then
        [ ! -L "$dest" ] || die "refusing symlink vhost destination: $name"
        record_warning nginx_vhost \
            "TLS lineage preservation is unavailable on the installed framework; installing repository vhost bytes verbatim for ${name}"
        trusted_change rendered-host-config "$dest" -- cp -f "$src" "$dest"
        return 0
    fi
...
# Which interpreter can import mojo.deploy.vhost_install? Unlike the heredoc it
# replaced, the module is a real import: it pulls in mojo/__init__.py, which
# pulls django.http and objict. Two candidates, then a fallback.
#
#   1. MOJOSEC_PYTHON with the MojoSec flags — the hermetic first choice
#      (-E drops a PYTHONPATH-injected copy, -P drops cwd), neither of which
#      affects site-packages resolution for -m.
#   2. bare python3 -E — the interpreter pip installed the framework into,
#      proven working in THIS run by the render step above, which dies if
#      `python3 -m mojo.deploy render` fails.
#   3. neither — install_file semantics plus a deploy warning. This is the
#      framework-DOWNGRADE case too: this file's inode outlives the pip
#      install (see the header), so a newer post_deploy.sh can be running
#      against older site-packages that have no such module.
#
# Deliberately an `if`, never a `case $rc in ... *) die`: a wedged
# mojo/__init__.py makes the import itself raise, and that must read as
# "absent", not "fatal". This helper must never become a new veto.
VHOST_PY=""
VHOST_PY_FLAGS=(-E)
if (cd / && "$MOJOSEC_PYTHON" "${MOJOSEC_PY_FLAGS[@]}" \
        -c 'import mojo.deploy.vhost_install' >/dev/null 2>&1); then
    VHOST_PY="$MOJOSEC_PYTHON"
    VHOST_PY_FLAGS=("${MOJOSEC_PY_FLAGS[@]}")
elif (cd / && python3 -E -c 'import mojo.deploy.vhost_install' >/dev/null 2>&1); then
    VHOST_PY="python3"
    VHOST_PY_FLAGS=(-E)
fi
[ -n "$VHOST_PY" ] \
    || log "  vhost TLS preservation unavailable (no interpreter can import mojo.deploy.vhost_install)"
