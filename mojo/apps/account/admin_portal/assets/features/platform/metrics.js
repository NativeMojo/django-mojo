...
export async function metricsPage(ctx, signal) {
  // The lane, not the raw grant: the bootstrap decides what this feature may
  // show, and every other Platform route reads its capabilities the same way.
  ...
  const state = {
    account: CATEGORIES[route.state.tab] ? route.state.tab : DEFAULTS.account,
  ...
  const listSlot = h('div', {class: 'row-page metrics-resources'});
  ...
  function rememberRoute() {
    history.replaceState({}, '', routeHref('metrics', {
      tab: state.account, category: state.category, focus: state.focus || '',
    }));
  }
