"""
Public auth-config endpoint.

Lets a custom front-end fetch the resolved auth config (theme + which
registration and login methods are offered) for a group, so it can render its
own login/registration UI. The framework-hosted bouncer pages render
server-side and do not need this.
"""
from mojo import decorators as md
from mojo.helpers.response import JsonResponse
from mojo.apps.account.services import auth_config


@md.GET("auth/config")
@md.public_endpoint("Resolved auth config (theme + offered methods)")
def on_auth_config(request):
    """Return the resolved, public-subset auth config for an optional
    `group_uuid` — the same config the hosted auth pages render from."""
    group = auth_config.resolve_group_from_request(request)
    cfg = auth_config.resolve_auth_config(group=group, request=request)
    return JsonResponse({
        "status": True,
        "data": auth_config.public_auth_config(cfg),
    })
