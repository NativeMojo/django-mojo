...
_JTI_KEYS = {
    ...
    KIND_FORCED_PASSWORD: "forced_password_jti",
}
_TS_KEYS = {
    ...
    KIND_FORCED_PASSWORD: "forced_password_ts",
}
...
def _token_ttl(kind):
    ...
    ttl_map = {
        ...
        KIND_FORCED_PASSWORD: settings.get("FORCED_PASSWORD_TOKEN_TTL", 600),
    }
...
def _verify(raw_token, expected_kind=None, consume=True):
    """
...
def generate_forced_password_token(user):
    """Generate a short-lived single-use forced-password credential."""
    return _generate(user, KIND_FORCED_PASSWORD)


def verify_forced_password_token(token, consume=True):
    """Verify a ``tp:`` credential, optionally leaving it for atomic consume."""
    return _verify(
        token, expected_kind=KIND_FORCED_PASSWORD, consume=consume)
