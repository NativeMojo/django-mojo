// Infrastructure ▸ Maintenance — pending managed-service upgrades and the
// framework this installation runs, with the controls to apply them. Ported
// from v1's Maintenance page with its confirmations, its polling and its
// blocked-reason copy intact.
//
// Two rules the page never breaks:
//   1. Every confirmation states the consequence in plain words and demands
//      the identifier typed back.
//   2. Success is reported on the ENGINE VERSION, never on the status: a
//      resource back at "available" still on its old version did NOT upgrade.
...
export async function maintenanceTab(ctx, signal = null) {
  ...
  const body = h('div', {class: 'maintenance-body'}, loadingState('Loading…'));
  const root = h('div', {class: 'maintenance-page'}, body);
  ...
  // † and human-input in one control: the typed-echo confirmation comes first
  // (nothing paints while a human is reading it), and the moment they confirm,
  // note() replaces this row with its own progress line. There is no surviving
  // node to pin a pending state to, so the wrapper is headless and contributes
  // only the guard — one modal per row, per click.
  ...
  function frameworkRow() {
    ...
    if (framework.can_update) {
      ...
      link?.addEventListener('click', (event) => {
        event.preventDefault();
        runAction(null, () => applyFrameworkUpdate(),
          {key: 'infrastructure:framework-update'});
      });
      return row;
    }
