    ...
    if (trusted_journal_source(record, "sudo") and
        ...
        if match:
            ...
            attributes = build_evidence("auth.sudo_command", {
                ...
                "producer_uid": canonical_uid(record.get("_UID")),
                "producer_pid": canonical_uid(record.get("_PID")),
                "producer_comm": record.get("_COMM"),
                "producer_exe": record.get("_EXE"),
                "audit_loginuid": canonical_uid(record.get("_AUDIT_LOGINUID")),
                "systemd_unit": record.get("_SYSTEMD_UNIT"),
                "cgroup": record.get("_SYSTEMD_CGROUP"),
                "selinux": record.get("_SELINUX_CONTEXT"),
                "monotonic": _canonical_int(
                    record.get("__MONOTONIC_TIMESTAMP"), 0, 2 ** 63 - 1,
                    "monotonic", optional=True),
